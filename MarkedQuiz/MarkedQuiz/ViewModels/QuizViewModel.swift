...
final class QuizViewModel {
    ...
    func nextQuestion() {
        if let q = currentQuestion, q.type == "fill_blank" {
            saveFillBlankAnswer()
        }
        fillBlankText = ""
        if currentQuestionIndex < totalQuestions - 1 {
            currentQuestionIndex += 1
            // Load existing fill-blank answer if going back
            if let q = currentQuestion, q.type == "fill_blank", let existing = userAnswers[q.id] {
