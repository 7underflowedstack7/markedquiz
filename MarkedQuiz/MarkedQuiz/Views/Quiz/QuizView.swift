    ...
    private var fillBlankInput: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack {
                ...
                TextField("Type your answer...", text: $viewModel.fillBlankText)
                    .font(CRT.monoText(16))
                    .foregroundStyle(CRT.orangeGlow)
    ...
    private func answerOptions(_ question: Question) -> some View {
        VStack(spacing: 10) {
            ForEach(question.options) { option in
                answerOptionButton(question: question, option: option)
            }
        }
        .padding(.horizontal, 20)
    }

    private func answerOptionButton(question: Question, option: QuestionOption) -> some View {
        ...
        return Button {
            ...
            HStack(spacing: 12) {
                ...
                ZStack {
                    Circle()
                        .stroke(isSelected ? CRT.orangeBright : CRT.orangeFaint, lineWidth: 2)
                ...
                Text(option.id.uppercased())
                    .font(CRT.monoBold(14))
                    .foregroundStyle(isSelected ? CRT.orangeBright : CRT.orangeDim)
                    .frame(width: 20)

                Text(option.text)
                    .font(CRT.monoText(14))
                    .foregroundStyle(isSelected ? CRT.orangeGlow : CRT.orangeBright)
                    .multilineTextAlignment(.leading)

                Spacer()
            }
            .padding(14)
            .background(isSelected ? CRT.orangeBright.opacity(0.08) : CRT.bgPanel)
            .clipShape(RoundedRectangle(cornerRadius: 8))
            .overlay(
                RoundedRectangle(cornerRadius: 8)
                    .stroke(isSelected ? CRT.orangeBright.opacity(0.4) : CRT.orangeFaint, lineWidth: 1)
            )
        }
